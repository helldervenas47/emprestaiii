...
/**
 * Seção exibida dentro do diálogo de edição da receita.
 * O vínculo receita↔boleto é 1:1 e iniciado pela receita.
 */
export function IncomeBoletoLinkSection({ incomeId }: Props) {
  const { items, loading, linkIncome, unlinkIncome } = useMyBoletos();
  ...
  const linked = useMemo(
    () => items.find((b) => b.income_id === incomeId) ?? null,
    [items, incomeId],
  );
  ...
  const available = useMemo(() => {
    ...
    return items
      .filter((b) => !b.income_id && !b.expense_id)
      .filter(
        (b) =>
          !q ||
  ...
  const handleLink = async (boletoId: string) => {
    ...
    try {
      if (linked && linked.id !== boletoId) {
        await unlinkIncome(linked.id);
      }
      await linkIncome(boletoId, incomeId);
    ...
    <div className="rounded-lg border border-border/60 bg-muted/30 p-3 space-y-2">
      ...
      {linked ? (
        ...
        <div className="text-[11px] text-muted-foreground">
          Nenhum boleto vinculado. Cada receita pode ter até 1 boleto.
        </div>
      )}
      ...
      <Dialog open={pickerOpen} onOpenChange={setPickerOpen}>
        ...
        >
          <DialogHeader>
            <DialogTitle>
              {linked ? "Substituir boleto vinculado" : "Vincular boleto à receita"}
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button type="button" size="sm" variant="outline" className="h-9 px-2 gap-1 shrink-0" onClick={() => setCreateOpen(true)}>
              <Plus className="h-4 w-4" /> Novo
            </Button>
          </div>

          <div className="text-[11px] text-muted-foreground">
            Apenas boletos sem vínculo com despesa ou outra receita são exibidos.
          </div>
      ...
      <BoletoFormDialog
        ...
        onSaved={async (newId) => {
          try {
            if (linked && linked.id !== newId) await unlinkIncome(linked.id);
            await linkIncome(newId, incomeId);
