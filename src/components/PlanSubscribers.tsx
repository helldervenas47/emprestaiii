...
export function PlanSubscribers() {
  ...
  const fetchSubscribers = async () => {
    ...
    // Fetch subscriptions (admin can see via service role through edge function or direct if admin)
  ...
  if (isMobile) {
    return (
      <div className="space-y-2">
        {subscribers.map((sub) => {
          const isExpanded = expandedId === sub.user_id;
          return (
            <Card key={sub.user_id + sub.environment} className="overflow-hidden">
              <button
                className="w-full flex items-center justify-between p-3 text-left"
                onClick={() => setExpandedId(isExpanded ? null : sub.user_id)}
              >
                <div className="min-w-0">
                  <p className="text-sm font-medium text-foreground truncate">
                    {sub.display_name || sub.user_id.slice(0, 8)}
                  </p>
                  <p className="text-xs text-muted-foreground">{sub.product_id}</p>
                </div>
              ...
              {isExpanded && (
                <div className="px-3 pb-3 space-y-1 border-t border-border/30 pt-3 text-sm">
                  <p className="text-muted-foreground"><span className="font-medium text-foreground">Plano:</span> {sub.product_id}</p>
    ...
    <Card>
      <CardContent className="p-0">
        <Table>
          ...
          <TableBody>
            {subscribers.map((sub) => (
              <TableRow key={sub.user_id + sub.environment}>
                <TableCell className="font-medium">
                  {sub.display_name || sub.user_id.slice(0, 8)}
                </TableCell>
                <TableCell className="text-muted-foreground">{sub.product_id}</TableCell>
                <TableCell className="text-muted-foreground">{sub.price_id}</TableCell>
                ...
                <TableCell className="text-muted-foreground">
                  ...
                  {sub.cancel_at_period_end && (
                    <span className="text-xs text-destructive ml-1">(cancela)</span>
                  )}
                </TableCell>
              </TableRow>
            ))}
