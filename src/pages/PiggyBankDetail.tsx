...
export default function PiggyBankDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Detalhes do rendimento — indexados por data (YYYY-MM-DD).
      // A view `vw_cofrinho_rendimento_por_dia` já agrega por (cofrinho_id, data),
      // então mapeamos 1:1, apenas aplicando piso em zero por segurança.
      ...
      if (!yieldRes.error && Array.isArray(yieldRes.data)) {
        for (const r of yieldRes.data as any[]) {
          ...
          detailsByDate[date] = {
            ...
            rendimentoBruto: Math.max(0, Number(r.rendimento_bruto_dia || 0)),
            iof: Math.max(0, Number(r.iof_dia || 0)),
            impostoRenda: Math.max(0, Number(r.imposto_renda_dia || 0)),
            rendimentoLiquido: Math.max(0, Number(r.rendimento_liquido_dia || 0)),
