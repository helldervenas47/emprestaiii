...
#   SUPABASE_PROJECT_REF   ref do projeto externo (ex.: abcd1234...)
...
: "${SUPABASE_PROJECT_REF:?defina SUPABASE_PROJECT_REF (ref do projeto Supabase externo)}"
...
echo "→ Linkando projeto $SUPABASE_PROJECT_REF..."
supabase link --project-ref "$SUPABASE_PROJECT_REF" >/dev/null

if [ -n "${GEMINI_API_KEY:-}" ]; then
  echo "→ Atualizando secret GEMINI_API_KEY..."
  supabase secrets set GEMINI_API_KEY="$GEMINI_API_KEY" --project-ref "$SUPABASE_PROJECT_REF" >/dev/null
fi
...
echo "Endpoint: https://${SUPABASE_PROJECT_REF}.supabase.co/functions/v1/${FUNCTION_NAME}"
